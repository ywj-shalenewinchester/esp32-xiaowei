...
LvglDisplay::LvglDisplay() {
    // 1. 实例化动态字形缓存（用于在 SRAM/PSRAM 之间按需分配字体内存）
    dynamic_glyph_cache_ = std::make_unique<DynamicGlyphCache>();
    // 2. 创建一个硬件定时器，用于“几秒后自动隐藏通知栏”（默认 5 秒）
    esp_timer_create_args_t notification_timer_args = {
        .callback =
            [](void* arg) {
                LvglDisplay* display = static_cast<LvglDisplay*>(arg);
                DisplayLockGuard lock(display);     // LVGL 必须加锁操作
                // 时间到了，隐藏通知，恢复显示默认状态栏
    ...
    // 3. 创建电源管理锁
    // 获取 ESP_PM_APB_FREQ_MAX 锁，确保在更新屏幕时，APB 总线频率保持最高，防止 DMA 传输时饿死或花屏
...
// 添加动态字形（大模型返回生僻字时，动态把该字的位图塞给 LVGL
...
// 设置电源保存模式 (开启/关闭)
// 开启后，会自动切换到低功耗模式，关闭后会恢复到正常模式
// 注意：这不会影响到大模型的运行，只是在显示上会切换到低功耗模式
