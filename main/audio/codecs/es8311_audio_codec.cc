...
void Es8311AudioCodec::SetOutputVolume(int volume) {
    ...
    // 钳制到 0-100：esp_codec_dev_set_out_vol 按 0-100 百分比音量处理，
    // 内部经默认曲线 (0% -> -50 dB, 100% -> 0 dB) 换算后写入 ES8311 DAC 音量寄存器 (0x32)
    if (volume < 0) {
        volume = 0;
    }
    if (volume > 100) {
    ...
    // dev_ 未创建时(输出未启用)只保存，下次 UpdateDeviceState 打开设备时会
    // 用 output_volume_ 重新应用 (见 UpdateDeviceState 中的 set_out_vol 调用)
